# This Makefile is for the Roguelike-Utils extension to perl.
#
...
#     ABSTRACT => q[Roguelike Libary for Perl]
#     AUTHOR => q[Erik Aronesty (erik@q32.com)]
#     NAME => q[Roguelike-Utils]
...
DISTNAME = Roguelike-Utils
DISTVNAME = Roguelike-Utils-0.4.132
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Roguelike-Utils' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      0.4.132' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,4,132,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Roguelike Libary for Perl</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Erik Aronesty (erik@q32.com)</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  test2.pl $(INST_LIB)/test2.pl \
	  eraseme.pl $(INST_LIB)/eraseme.pl 
